...
public class DataLinkController {
	...
	public String getProduct(Long productId) {
		...
		String dimProductJSON = jedis.get("dim_product_" + productId);//读redis主集群
		
		if(dimProductJSON == null || "".equals(dimProductJSON)) {
	    	String productDataJSON = eshopProductService.findProductById(productId);//通过fegion拉取依赖服务的接口
	    	...
	    	if(productDataJSON != null && !"".equals(productDataJSON)) {
	    		...
	    		jedis.set("dim_product_" + productId, productDataJSONObject.toJSONString());//将数据写入redis主集群
